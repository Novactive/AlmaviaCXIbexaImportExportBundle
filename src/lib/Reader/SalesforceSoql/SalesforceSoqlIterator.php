...
class SalesforceSoqlIterator implements ReaderIteratorInterface, SeekableIterator
{
    ...
    /**
     * @param \AlmaviaCX\Bundle\IbexaImportExport\Salesforce\SalesforceApiClient      $apiClient
     * @param \AlmaviaCX\Bundle\IbexaImportExport\Salesforce\SalesforceApiCredentials $credentials
     * @param string                                                                  $domain
     * @param string                                                                  $version
     * @param string                                                                  $queryString
     * @param string                                                                  $countQueryString
    ...
    protected function fetch(): SoqlResultsIterator
    {
        if (!$this->cache->containsKey('results_iterator')) {
            ...
            $queryId = null;
            $batchSize = null;
            if (preg_match('#/services/data/v([\d.]+)/query/([\w]+)-([\d]+)$#', $nextRecordsUrl, $matches)) {
                $queryId = $matches[2];
                $batchSize = (int) $matches[3];
            }

            $this->cache->set('results_iterator', new SoqlResultsIterator(
